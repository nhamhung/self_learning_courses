...
def accuracy(typed, source):
    ...
    return (match_counts / len(typed_words)) * 100.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    lowest_index, lowest_difference = float('inf'), float('inf')
...
def feline_fixes(typed, source, limit):
    ...
    if typed[0] != source[0]:
        if limit == 0:  # difference > limit, just count 1 diff
            return 1

        difference = 1 + feline_fixes(typed[1:], source[1:], limit - 1)
    else:
        difference = feline_fixes(typed[1:], source[1:], limit)
...
def minimum_mewtations(typed, source, limit):
    ...
    if limit < 0:
        ...
        add = minimum_mewtations(
            typed, source[1:], limit - 1)  # Fill in these lines
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(source)):
        if i >= len(typed) or typed[i] != source[i]:
            break
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    result = [[] for _ in range(len(player_indices))]

    for word_index in word_indices:
        fastest_player_index = None
        ...
        result[fastest_player_index].append(get_word(match, word_index))
